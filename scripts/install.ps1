...
function Get-SelfHostRef {
    if ($env:AURA_SELFHOST_REF) {
        return $env:AURA_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = aura version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing AuraManager CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "AuraManager requires a 64-bit Windows installation."
    }
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "aura.zip") -UseBasicParsing
    } catch {
    ...
    try {
        ...
        $zipFile = Join-Path $tmpDir "aura.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "aura-cli-$version-windows-$arch.zip"
        $legacyAsset = "aura_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "aura.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".aura\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "aura.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "aura.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "aura.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "aura.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "AuraManager CLI installed to $binDir\aura.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "aura") {
        ...
        if ($isUpToDate) {
            Write-Ok "AuraManager CLI is up to date ($currentVer)"
            return
        }

        Write-Info "AuraManager CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "AuraManager CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "aura")) {
        Write-Fail "CLI installed but 'aura' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. AuraManager self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:AURA_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up AuraManager server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning AuraManager repository..."
    ...
    Write-Info "Pulling official AuraManager images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting AuraManager services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "AuraManager server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  AuraManager - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] AuraManager CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     aura setup               " -NoNewline; Write-Host "# Connect to AuraManager Cloud (AuraManager cloud)" -ForegroundColor DarkGray
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  AuraManager - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] AuraManager server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     aura setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping AuraManager services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No AuraManager installation found at $InstallDir"
    }

    if (Test-CommandExists "aura") {
        try {
            aura daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:AURA_MODE) { $env:AURA_MODE.ToLower() } else { "default" }
